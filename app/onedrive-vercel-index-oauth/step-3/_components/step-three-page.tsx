...
export default function StepThreePage({ accessToken, expiryTime, refreshToken, error, description, errorUri }:{ accessToken : string | null, expiryTime: number | null, refreshToken: string | null, error: string | null, description : string | null, errorUri: string | null }) {
  ...
  console.log(expiryTimeLeft, accessToken, refreshToken, expiryTime)

  if (!expiryTimeLeft || !accessToken || !refreshToken || !expiryTime) return null
  ...
  const sendAuthTokensToServer = async () => {
    ...
    await sendTokenToServer(accessToken, refreshToken, expiryTime.toString())
      .then(() => {
        setButtonError(false)
        setButtonContent(
          <div>
